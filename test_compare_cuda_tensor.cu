...
int main() {
    ...
    thrust::host_vector<double> h_a_vec(M*K);
    thrust::host_vector<double> h_b_vec(K*N);

    fill_random_double_values(h_a_vec.data(), h_a_vec.size(), random_engine);
    fill_random_double_values(h_b_vec.data(), h_b_vec.size(), random_engine);
    
    thrust::device_vector<double> d_a_vec = h_a_vec;
    thrust::device_vector<double> d_b_vec = h_b_vec;
    thrust::device_vector<double> d_c_vec(M*N);
    ...
    for (int i = 0; i < numIterations; ++i) {
        ...
        gemm_naive<double><<<threadNum, blockNum>>>(d_a_vec.data().get(), d_b_vec.data().get(), d_c_vec.data().get(), M, K, N);
    ...
    thrust::host_vector<double> h_naive_c_vec = d_c_vec;
    ...
    for (int i = 0; i < numIterations; ++i) {
        ...
        gemm_CUDA<double><<<threadNum, blockNum>>>(d_c_vec.data().get(), d_a_vec.data().get(), d_b_vec.data().get(), M, N, K);
    ...
    thrust::host_vector<double> h_c_vec = d_c_vec;
